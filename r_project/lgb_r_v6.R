...
train_8_path <- "/home/paperspace/data/talkingdata/train_sep_8_added.csv"
...
train_8 <- fread(train_8_path, drop = c("day", "ip"),
               colClasses = list(numeric=9:15))

dtrain <- rbind(dtrain, train_8[hour %in% 4:14])
...
## save
lgb.save(model, "mod/lgb_v6")
model <- lgb.load("mod/lgb_v6")
...
fwrite(sub, "sub/lightgbm_r_v6.csv")
