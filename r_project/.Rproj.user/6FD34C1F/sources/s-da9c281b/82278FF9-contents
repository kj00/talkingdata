...
##
train_one_add <- data.table()
for (ii in 6:8) {
  
train_one <- fread(
  paste0("/home/paperspace/data/talkingdata/train_sep_", ii, "_added.csv")
)
train_one <- train_one[is_attributed == 1]

train_one_add <- rbind(train_one_add, train_one)
rm(train_one)
}
...
#
dtrain <- rbind(train[ train.index,], train_one_add)
...
fwrite(sub, "sub/lightgbm_r_v3_wo_agg_whole_test.csv")
...
print(object.size(train_sep),units="auto")
